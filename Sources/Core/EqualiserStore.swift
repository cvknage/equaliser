...
final class EqualiserStore: ObservableObject {
    ...
    init(persistence: AppStatePersistence = AppStatePersistence()) {
        ...
        // Observe output device list changes (device connect/disconnect)
        deviceManager.$outputDevices
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in
      ...
      @objc private func handleSystemDefaultOutputChange() {
          ...
          // CRITICAL: When user switches away from our driver to another device,
          // macOS may hide our driver from CoreAudio. We must IMMEDIATELY set
          // the driver as default to keep it visible before reconfiguring.
          
          // Rename driver to match new output device first
          ...
          logger.info("Renamed driver to: \(driverName)")
           ...
           // Set driver as macOS default to maintain visibility
           isAppSettingSystemDefault = true
           
           if DriverManager.shared.setAsDefaultOutputDevice() {
                logger.info("Driver set as default to maintain visibility during reconfiguration")
              ...
              DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak self] in
               ...
               // Driver might not be visible yet - use async retry
               logger.warning("Driver not immediately visible, attempting to restore visibility")
               
               Task { @MainActor in
                   // Use async retry with initial delay for CoreAudio stabilization
                   if await DriverManager.shared.findDriverDeviceWithRetry() != nil {
                       // Now try to set as default
                       if DriverManager.shared.setAsDefaultOutputDevice() {
                           logger.info("Driver restored and set as default after retry")
                           self.isAppSettingSystemDefault = false
                           // Update output device and reconfigure
                           self.selectedOutputDeviceID = newDefault.uid
                           self.reconfigureRouting()
                       } else {
                           logger.error("Failed to set driver as default after retry")
                           self.routingStatus = .error("Driver unavailable")
                           self.isAppSettingSystemDefault = false
                       }
                   } else {
                       logger.error("Driver did not become visible - cannot reconfigure")
                       self.routingStatus = .driverNotInstalled
                       self.showDriverPrompt = true
                       self.isAppSettingSystemDefault = false
                   }
          ...
          if newDefault.uid == selectedOutputDeviceID {
           ...
           if let current = selectedOutputDeviceID,
              ...
              current != newDefault.uid {
               // Remove any older occurrences of this device
               self.outputDeviceHistory.removeAll { $0 == current }
               // Add to front of history
               self.outputDeviceHistory.insert(current, at: 0)
               // Limit history size
               if self.outputDeviceHistory.count > 10 {
                   self.outputDeviceHistory.removeLast()
               }
               self.logger.debug("Added to output history, count: \(self.outputDeviceHistory.count)")
           }
      ...
      /// Handles changes to the output device list (device connect/disconnect).
      /// Checks if currently selected output device was removed and switches to alternative.
      private func handleOutputDevicesChanged() {
          ...
          let selectedExists = deviceManager.outputDevices.contains { $0.uid == selectedOutputDeviceID }
          
          guard !selectedExists else {
          ...
          for uid in outputDeviceHistory {
              if let device = deviceManager.device(forUID: uid),
                 !device.isVirtual {
                  logger.info("Restoring output from history: \(device.name)")
          ...
          // Last resort: first available non-virtual output
          if let fallback = deviceManager.outputDevices.first(where: { !$0.isVirtual }) {
              logger.info("Switching to fallback: \(fallback.name)")
              selectedOutputDeviceID = fallback.uid
              
              let driverName = "\(fallback.name) (Equaliser)"
              DriverManager.shared.setDeviceName(driverName)
    ...
    func reconfigureRouting() {
        ...
        if manualModeEnabled {
            ...
            // If not visible (hidden during device transition), wait for it
