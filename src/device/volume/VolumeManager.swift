...
final class VolumeManager: ObservableObject {
    ...
    /// macOS may restore stored volume multiple times when device becomes default.
    func scheduleDriftChecks() {
        for delay in Constants.driftCheckDelays {
            DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self] in
                self?.checkAndResyncDrift(delay: delay)
            }
        }
    }

    /// Checks for volume drift and re-syncs driver to output device volume.
    private func checkAndResyncDrift(delay: TimeInterval) {
        guard let driverID = driverDeviceID,
              let outputID = outputDeviceID,
              let expectedVolume = volumeService.getDeviceVolumeScalar(deviceID: outputID),
